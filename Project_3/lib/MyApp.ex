defmodule MyApp do
  ...
  def start(_type, _args) do
    ...
    check_if_finished(counter_pid,numNodes*numRequests-numRequests)

    ret_value
  end

  def check_if_finished(counter_pid, threshold) do
    
    #IO.puts "The max hop value is #{inspect GenServer.call(counter_pid,{:answer})}"
    {suc, maxHops} = GenServer.call(counter_pid,{:answer})
    if suc >= threshold do
      IO.puts "The max hop value is #{maxHops}"
    else
      check_if_finished(counter_pid,threshold)
    end
  end
